#!/usr/bin/env bash
# Topology compiled from topology/simple.yaml
set -e
# Create the agent(s)
...
make node/up IPC_AGENT_NR=0 IPC_NODE_NR=1 IPC_PARENT_NR=0 IPC_WALLET_NR=0 IPC_SUBNET_NAME=thorax
make connect IPC_AGENT_NR=0 IPC_NODE_NR=1
make node/up IPC_AGENT_NR=0 IPC_NODE_NR=2 IPC_PARENT_NR=1 IPC_WALLET_NR=0 IPC_SUBNET_NAME=abdomen
